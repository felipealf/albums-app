...
export default function PhotosPage() {
  ...
  useEffect(() => { navigation.setOptions({ title: albumTitle ? `Fotos · ${albumTitle}` : "Fotos" }); }, [navigation, albumTitle]);

  if (loading) return (<ScreenContainer><Header title="Fotos" subtitle="Carregando..." /><ActivityIndicator size="large" /></ScreenContainer>);
  if (error) return (<ScreenContainer><Header title="Fotos" /><ErrorState error={error.message} onRetry={onRefresh} /></ScreenContainer>);
