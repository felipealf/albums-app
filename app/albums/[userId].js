...
export default function AlbumsPage() {
  ...
  useEffect(() => { navigation.setOptions({ title: userName ? `Álbuns · ${userName}` : "Álbuns" }); }, [navigation, userName]);

  if (loading) return (<ScreenContainer><Header title="Álbuns" subtitle="Carregando..." /><ActivityIndicator size="large" /></ScreenContainer>);
  if (error) return (<ScreenContainer><Header title="Álbuns" /><ErrorState error={error.message} onRetry={onRefresh} /></ScreenContainer>);
  ...
  return (
    <ScreenContainer>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Card onPress={() => router.push({ pathname: `/photos/${item.id}`, params: { albumTitle: item.title } })}>
